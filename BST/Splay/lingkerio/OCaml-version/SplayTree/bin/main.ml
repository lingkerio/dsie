let process_operation tree (operation, value) =
  ...
  | "insert" -> Splay.insert value tree
  | "search" ->
    let (_, new_tree) = Splay.member value tree in
    new_tree
  | "delete" -> Splay.delete value tree
  | "findmin" -> 
    let (_, tree) = Splay.min_splay tree in
    tree
  | "findmax" ->
    let (_, tree) = Splay.max_splay tree in
    tree
  | _ -> tree
...
let () =
  ...
  let (_, elapsed_time) =
    time_execution (Array.fold_left process_operation Splay.empty) operations
  in
