...
AVLNode* process_operation(AVLNode* root, const char* operation, int value, FILE* output_file) {
    if (strcmp(operation, "insert") == 0) {
        ...
        AVLNode* node_to_delete = search(root, value) ? root : NULL;
        ...
        AVLNode* min_node = find_min(root);
        if (min_node) {
        ...
        AVLNode* max_node = find_max(root);
        if (max_node) {
